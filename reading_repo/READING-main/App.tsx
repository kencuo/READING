...
import StatsErrorBoundary from './components/StatsErrorBoundary';
...
import { BUILT_IN_TUTORIAL_BOOK_ID } from './utils/builtInTutorialBook';
...
const READER_MODE_MIGRATION_KEY = 'app_reader_mode_migration_v1';
...
const normalizeAppSettings = (raw: unknown): AppSettings => {
  ...
  const readerMore = {
    appearance: {
      readingMode:
        appearanceSource.readingMode === 'scroll' || appearanceSource.readingMode === 'page'
          ? appearanceSource.readingMode
          : DEFAULT_READER_MORE_SETTINGS.appearance.readingMode,
      readerThemeId:
        typeof appearanceSource.readerThemeId === 'string' &&
        ['beige', 'green', 'blue', 'pink', 'peach', 'night'].includes(appearanceSource.readerThemeId)
          ? (appearanceSource.readerThemeId as AppSettings['readerMore']['appearance']['readerThemeId'])
          : DEFAULT_READER_MORE_SETTINGS.appearance.readerThemeId,
      readerFabOffsetPx:
        typeof appearanceSource.readerFabOffsetPx === 'number' && Number.isFinite(appearanceSource.readerFabOffsetPx)
          ? Math.min(200, Math.max(0, Math.round(appearanceSource.readerFabOffsetPx)))
          : DEFAULT_READER_MORE_SETTINGS.appearance.readerFabOffsetPx,
      bubbleFontSizeScale:
...
const stripBuiltInSampleBooks = (books: Book[]): Book[] => {
  return books.filter(book => {
    ...
    const isBuiltInTutorial = book.id === BUILT_IN_TUTORIAL_BOOK_ID;
    return !(isLegacySampleId && isLegacySampleCover) && !isBuiltInTutorial;
  });
};
...
const App: React.FC = () => {
  ...
  const resetStatsData = useCallback(() => {
    const statsKeys = [
      'app_stats_goal_book_ids',
      'app_stats_sticky_note_cache_v1',
      'app_stats_note_history_v1',
      DAILY_READING_MS_STORAGE_KEY,
      READING_MS_BY_BOOK_ID_STORAGE_KEY,
      COMPLETED_BOOK_IDS_STORAGE_KEY,
      COMPLETED_BOOK_REACHED_AT_STORAGE_KEY,
    ];
    try {
    ...
    setDailyReadingMsByDate({});
    setReadingMsByBookId({});
    setCompletedBookIds([]);
    setCompletedAtByBookId({});
  ...
  const [userSignature, setUserSignature] = useState(() => {
    const legacySignature = "黑夜无论怎样漫长 白昼总会到来";
    const previousDefaultSignature = "历史是一堆灰烬，然灰烬深处有余温吗";
    const defaultSignature = "灰烬深处有余温";
    // Check for null strictly so we allow empty string as a valid signature
    const saved = localStorage.getItem('app_user_signature');
    if (saved === null) return defaultSignature;
    return (saved === legacySignature || saved === previousDefaultSignature) ? defaultSignature : saved;
  });
  ...
  const handleSelectPersona = (personaId: string | null) => {
    ...
    if (!persona) return;
    const boundIds = (persona.boundCharacterIds || []).filter(Boolean);
    if (boundIds.length === 0) return;
    if (activeCharacterId && boundIds.includes(activeCharacterId)) return;
    const match = characters.find((char) => boundIds.includes(char.id));
    if (match) setActiveCharacterId(match.id);
  };

  const handleSelectCharacter = (characterId: string | null) => {
    setActiveCharacterId(characterId);
    if (!characterId) return;
    const boundPersona = personas.find((item) => (item.boundCharacterIds || []).includes(characterId));
    if (boundPersona && boundPersona.id !== activePersonaId) {
      setActivePersonaId(boundPersona.id);
    }
  };

  const applyBookBindings = (book: Book) => {
    const boundPersonaId = book.boundPersonaId;
    const boundCharacterId = book.boundCharacterId;
    const hasPersona = boundPersonaId && personas.some((item) => item.id === boundPersonaId);
    const hasCharacter = boundCharacterId && characters.some((item) => item.id === boundCharacterId);

    if (hasPersona && hasCharacter) {
      setActivePersonaId(boundPersonaId || null);
      setActiveCharacterId(boundCharacterId || null);
      return;
    }
    if (hasPersona && boundPersonaId) {
      handleSelectPersona(boundPersonaId);
      return;
    }
    if (hasCharacter && boundCharacterId) {
      handleSelectCharacter(boundCharacterId);
    }
  };
  ...
  useEffect(() => {
    ...
    const next = personas.map((persona) => {
      const boundIds = Array.isArray(persona.boundCharacterIds)
        ? persona.boundCharacterIds.filter(Boolean)
        : [];
      if (boundIds.length > 0) return persona;
      const legacyRoles = Array.isArray((persona as any).boundRoles)
        ? (persona as any).boundRoles
        : [];
      if (legacyRoles.length === 0) return persona;
      const mappedIds = characters
        .filter((char) => legacyRoles.includes(char.name))
        .map((char) => char.id);
      changed = true;
      return { ...persona, boundCharacterIds: mappedIds };
    });
    if (changed) setPersonas(next);
  }, [personas, characters]);
  ...
  useEffect(() => {
    try {
      if (localStorage.getItem(READER_MODE_MIGRATION_KEY)) return;
      const shouldMigrate = appSettings.readerMore.appearance.readingMode === 'page';
      localStorage.setItem(READER_MODE_MIGRATION_KEY, '1');
      if (shouldMigrate) {
        setAppSettings(prev => ({
          ...prev,
          readerMore: {
  ...
  }, [appSettings.readerMore.appearance.readingMode]);
  useEffect(() => {
    ...
    const runProactiveCheck = async (plannedAt: number) => {
      ...
      try {
        ...
        const result = await runConversationGeneration({
          ...
          characterWorldBookEntries: buildCharacterWorldBookSections(activeCharacter, worldBookEntries, targetBook.id),
  ...
  const appWrapperClass = `relative flex flex-col h-full font-sans overflow-hidden transition-colors duration-300 ${isDarkMode ? 'dark-mode ios-bg ios-text' : 'ios-bg ios-text'}`;
    ...
    >
      ...
      <div className={`flex-1 flex flex-col overflow-hidden relative ${viewAnimationClass}`}>
        ...
        {currentView === AppView.STATS && (
          <StatsErrorBoundary onReset={resetStatsData}>
            <Stats
              isDarkMode={isDarkMode}
              dailyReadingMsByDate={dailyReadingMsByDate}
      ...
      >
        <div className="jj-nav pointer-events-auto">
          <button 
            onClick={() => transitionToView(AppView.LIBRARY)}
            disabled={isViewTransitioning}
            className={`jj-nav-item ${currentView === AppView.LIBRARY ? 'jj-nav-active' : ''}`}
          >
            <LayoutGrid size={20} strokeWidth={currentView === AppView.LIBRARY ? 2.5 : 2} />
            <span>书架</span>
          </button>
          ...
          <button
            ...
            disabled={isViewTransitioning}
            className={`jj-nav-item ${currentView === AppView.STATS ? 'jj-nav-active' : ''}`}
          >
            <PieChart size={20} strokeWidth={currentView === AppView.STATS ? 2.5 : 2} />
            <span>统计</span>
          </button>
          ...
          <button
            ...
            disabled={isViewTransitioning}
            className={`jj-nav-item ${currentView === AppView.STUDY_HUB ? 'jj-nav-active' : ''}`}
          >
            <Sparkles size={20} strokeWidth={currentView === AppView.STUDY_HUB ? 2.5 : 2} />
            <span>学习</span>
          </button>
          ...
          <button
            ...
            disabled={isViewTransitioning}
            className={`jj-nav-item ${currentView === AppView.SETTINGS ? 'jj-nav-active' : ''}`}
          >
            <SettingsIcon size={20} strokeWidth={currentView === AppView.SETTINGS ? 2.5 : 2} />
            <span>设置</span>
          </button>
        </div>
      </nav>
