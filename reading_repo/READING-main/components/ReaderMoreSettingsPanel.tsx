...
const ReaderMoreSettingsPanel: React.FC<Props> = (props) => {
  ...
  const raisedCardClass = 'ios-card';
  ...
  const renderSummaryModal = () => {
    ...
    return (
      <div className={`fixed inset-0 z-[95] flex items-center justify-center p-6 ${modalClosing ? 'app-fade-exit' : 'app-fade-enter'}`}>
        ...
        <div className={`ios-panel relative w-full max-w-2xl max-h-[80vh] rounded-3xl p-5 flex flex-col ${modalClosing ? 'app-fade-exit' : 'app-fade-enter'}`}>
  ...
  const renderBgUrlModal = () => {
    ...
    return (
      <div className={`fixed inset-0 z-[95] flex items-center justify-center p-6 ${modalClosing ? 'app-fade-exit' : 'app-fade-enter'}`}>
        <button type="button" className="absolute inset-0 bg-black/45 backdrop-blur-sm" onClick={closeModal} />
        <div className={`ios-panel relative w-full max-w-md rounded-2xl p-6 ${modalClosing ? 'app-fade-exit' : 'app-fade-enter'}`}>
  ...
  return (
    <>
      <div
        className="absolute inset-0 z-[90] ios-bg"
        style={{
          paddingTop: `${Math.max(0, safeAreaTop || 0)}px`,
          paddingBottom: `${Math.max(0, safeAreaBottom || 0)}px`,
        }}
      >
        <div className={`h-full overflow-hidden overscroll-none ${closing ? 'app-view-exit-right' : 'app-view-enter-left'} ios-bg`} style={panelMotionStyle}>
          <div className="h-full flex flex-col">
            {/* Header */}
            <div className="flex items-center gap-3 p-4 z-10 ios-panel">
              ...
              <div className="relative grid grid-cols-4 overflow-visible">
                ...
                {TAB_ITEMS.map((it) => (
                  ...
                  >
                    ...
                    <span>{it.label}</span>
                    {tab === it.key && (
                      <div className="absolute bottom-0 left-0 right-0 h-1 ios-bg" />
                    )}
                  </button>
                ))}
              </div>
                  ...
                  <div className="space-y-0">
                    ...
                    <div className="py-5">
                      <div className="flex items-center justify-between">
                        <span className={`text-sm font-bold ${headingClass}`}>阅读模式</span>
                        <div className="flex items-center gap-2">
                          <button
                            type="button"
                            onClick={() => onUpdateAppearanceSettings({ readingMode: 'page' })}
                            className={`h-9 px-4 rounded-full text-xs font-bold transition-all ${appearanceSettings.readingMode === 'page' ? `${btnClass} ${activeBtnClass} text-rose-400` : `${pressedClass} text-slate-400`}`}
                          >
                            翻页
                          </button>
                          <button
                            type="button"
                            onClick={() => onUpdateAppearanceSettings({ readingMode: 'scroll' })}
                            className={`h-9 px-4 rounded-full text-xs font-bold transition-all ${appearanceSettings.readingMode === 'scroll' ? `${btnClass} ${activeBtnClass} text-rose-400` : `${pressedClass} text-slate-400`}`}
                          >
                            滚动
                          </button>
                        </div>
                    ...
                    {/* 阅读主题 */}
                    <div className="py-5">
                      <div className={`text-sm font-bold mb-3 ${headingClass}`}>阅读主题</div>
                      <div className="grid grid-cols-2 gap-2">
                        {READER_THEMES.map((theme) => (
                          <button
                            key={theme.id}
                            type="button"
                            onClick={() => onUpdateAppearanceSettings({ readerThemeId: theme.id })}
                            className={`rounded-xl p-2 border transition-all text-left ${appearanceSettings.readerThemeId === theme.id ? 'ring-2 ring-rose-300/60 border-rose-300/40' : 'border-transparent'}`}
                            style={{ background: theme.chromeColor }}
                          >
                            <div className="h-10 rounded-lg border border-white/70" style={{ background: theme.paperColor }} />
                            <div className="mt-2 text-xs font-semibold" style={{ color: theme.textColor }}>{theme.label}</div>
                          </button>
                        ))}
                      </div>
                    </div>
                    ...
                    {/* AI按钮位置 */}
                    <div className="py-5">
                      <div className={`text-sm font-bold mb-3 ${headingClass}`}>AI按钮位置</div>
                      <div className="flex items-center justify-between text-xs text-slate-500 mb-2">
                        <span>向上偏移</span>
                        <span>{appearanceSettings.readerFabOffsetPx}px</span>
                      </div>
                      <div className="relative h-2">
                        <div className={`absolute inset-0 rounded-full ${isDarkMode ? 'bg-slate-700' : 'bg-black/5'}`} />
                        <div className="absolute inset-y-0 left-0 rounded-full bg-rose-300" style={{ width: `${(appearanceSettings.readerFabOffsetPx / 200) * 100}%` }} />
                        <input
                          type="range"
                          min="0"
                          max="200"
                          step="5"
                          value={appearanceSettings.readerFabOffsetPx}
                          onChange={(e) => onUpdateAppearanceSettings({ readerFabOffsetPx: clampInt(Number(e.target.value || '0'), 0, 200) })}
      ...
      {pendingArchiveDelete && (
        <div className={`fixed inset-0 z-[96] flex items-center justify-center p-6 ${archiveDeleteDialogClosing ? 'app-fade-exit' : 'app-fade-enter'}`}>
          ...
          />
          <div className={`ios-panel relative w-full max-w-sm rounded-2xl p-6 ${archiveDeleteDialogClosing ? 'app-fade-exit' : 'app-fade-enter'}`}>
