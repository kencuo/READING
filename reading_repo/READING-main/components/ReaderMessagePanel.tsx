...
const ReaderMessagePanel: React.FC<ReaderMessagePanelProps> = ({
  ...
  const characterWorldBookEntries = useMemo(
    () => buildCharacterWorldBookSections(activeCharacter, worldBookEntries, activeBook?.id || null),
    [activeCharacter, worldBookEntries, activeBook?.id]
  );
  ...
  const fabOffsetPx = Math.min(200, Math.max(0, Math.round(readerMoreAppearance.readerFabOffsetPx || 0)));
    ...
    <>
      ...
      {!isAiPanelOpen && (
        <button
          ...
          style={{ bottom: `${fabBottomPx + fabOffsetPx}px`, color: 'rgb(var(--theme-400) / 1)' }}
        >
      ...
      >
        <div
          ref={aiPanelRef}
          className="rm-panel ios-panel absolute bottom-0 left-0 right-0 pointer-events-auto overflow-hidden rounded-t-3xl rounded-b-none"
          style={{
            height: `${resolvedPanelVisualHeight}px`,
          }}
        >
          ...
          <div className="rm-input-area p-4 pb-6 relative z-10" style={{ paddingBottom: `${24 + safeBottomInset}px` }}>
            {toast && (
              <div
                className={`absolute z-20 left-1/2 -translate-x-1/2 -top-8 px-6 py-2 rounded-full flex items-center gap-2 text-xs font-bold ios-panel ${toast.type === 'error' ? 'text-rose-400' : isDarkMode ? 'text-slate-200' : 'text-slate-600'}`}
              >
