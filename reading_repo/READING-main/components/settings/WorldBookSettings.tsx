...
import { WorldBookEntry, ThemeClasses, Character } from './types';
...
const WorldBookSettings: React.FC<WorldBookSettingsProps> = ({
  ...
  const bookTitleById = useMemo(
    () => new Map(books.map((book) => [book.id, book.title || '未命名书籍'])),
    [books]
  );
      ...
      <section className="space-y-3">
        ...
        <div className="space-y-4">
          {sectionEntries.map((entry, index) => {
            ...
            const boundCharacterNames = (entry.boundCharacterIds || [])
              .map((id) => characterNameById.get(id))
              .filter(Boolean) as string[];
            const boundBookTitles = (entry.boundBookIds || [])
              .map((id) => bookTitleById.get(id))
              .filter(Boolean) as string[];
            const hasBindings = boundCharacterNames.length > 0 || boundBookTitles.length > 0;
                  ...
                  <div className="space-y-3 mt-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <span className="text-xs font-bold text-slate-400 uppercase">绑定角色</span>
                        <MultiSelectDropdown
                          options={characters.map((char) => ({
                            value: char.id,
                            label: char.nickname ? `${char.nickname}（${char.name}）` : char.name,
                          }))}
                          selected={entry.boundCharacterIds || []}
                          onChange={(nextIds) => updateWorldBookEntry(entry.id, 'boundCharacterIds', nextIds)}
                          placeholder="不绑定（全部角色）"
                      ...
                      <div className="space-y-1">
                        <span className="text-xs font-bold text-slate-400 uppercase">绑定书籍</span>
                        <MultiSelectDropdown
                          options={books.map((book) => ({
                            value: book.id,
                            label: book.title || '未命名书籍',
                          }))}
                          selected={entry.boundBookIds || []}
                          onChange={(nextIds) => updateWorldBookEntry(entry.id, 'boundBookIds', nextIds)}
                          placeholder="不绑定（全部书籍）"
                          inputClass={inputClass}
                          cardClass={cardClass}
                          isDarkMode={isDarkMode}
                        />
                    ...
                    {hasBindings && (
                      <div className="mt-2 text-[11px] text-slate-400 space-y-1">
                        {boundCharacterNames.length > 0 && (
                          <div>绑定角色：{boundCharacterNames.join('、')}</div>
                        )}
                        {boundBookTitles.length > 0 && (
                          <div>绑定书籍：{boundBookTitles.join('、')}</div>
                        )}
                      </div>
                    )}
                  </>
