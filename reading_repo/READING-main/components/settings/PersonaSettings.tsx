...
const PersonaSettings: React.FC<PersonaSettingsProps> = ({ 
  ...
  const characterNameById = new Map(characters.map((char) => [char.id, char.name]));
    ...
    <div className={`flex-1 flex flex-col p-6 pb-28 overflow-y-auto no-scrollbar relative ${containerClass} ${animationClass}`}>
      ...
      <div className="flex flex-col gap-6">
        {personas.map(persona => {
          ...
          const boundNames = (persona.boundCharacterIds || []).map((id) => characterNameById.get(id)).filter(Boolean) as string[];
          return (
                      ...
                      <>
                        ...
                        <div className="flex flex-col gap-0.5">
                          <p className="text-xs text-rose-400 flex items-center gap-1">
                            <FeatherIcon size={12} />
                            绑定: {boundNames.length > 0 ? boundNames.join(', ') : '无'}
                          </p>
                        </div>
                      </>
                    ) : (
                ...
                <div className="space-y-4 animate-fade-in mt-2">
                   ...
                   <div className="space-y-2">
                     ...
                     <MultiSelectDropdown 
                        ...
                        selected={persona.boundCharacterIds || []}
                        onChange={(newRoles) => updatePersona(persona.id, 'boundCharacterIds', newRoles)}
