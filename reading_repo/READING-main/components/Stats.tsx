...
type SummaryCardKey = 'streak' | 'duration' | 'completed';
...
const Stats: React.FC<StatsProps> = ({
  ...
  const axisTextColor = isDarkMode ? '#a1a1aa' : '#6B635A';
  ...
  const safeBooks = useMemo(() => (Array.isArray(books) ? (books.filter(Boolean) as StatsBook[]) : []), [books]);
  const safePersonas = useMemo(() => (Array.isArray(personas) ? (personas.filter(Boolean) as Persona[]) : []), [personas]);
  const safeCharacters = useMemo(() => (Array.isArray(characters) ? (characters.filter(Boolean) as Character[]) : []), [characters]);
  const safeWorldBookEntries = useMemo(
    () => (Array.isArray(worldBookEntries) ? (worldBookEntries.filter(Boolean) as WorldBookEntry[]) : []),
    [worldBookEntries]
  );
  ...
  const normalizedCharacterNickname = safeTrim(activeCharacterNickname) || DEFAULT_CHARACTER_NICKNAME;
  const activePersona = useMemo(
    () => safePersonas.find((persona) => persona.id === activePersonaId) || null,
    [safePersonas, activePersonaId]
  );
  const activeCharacter = useMemo(
    () => safeCharacters.find((character) => character.id === activeCharacterId) || null,
  ...
  const characterWorldBookEntries = useMemo(() => {
    const rawBoundCategories = Array.isArray(activeCharacter?.boundWorldBookCategories)
      ? activeCharacter.boundWorldBookCategories
      : [];
    const boundCategories = new Set(
      rawBoundCategories
        .map((category) => safeTrim(category))
        .filter(Boolean)
    );
    ...
    const scopedEntries = safeWorldBookEntries.filter((entry) => boundCategories.has(entry.category));
    return {
  ...
  }, [activeCharacter, safeWorldBookEntries]);
  ...
  useEffect(() => {
    ...
    const validBookIds = new Set(safeBooks.map((book) => book.id));
    setGoalBookIds((prev) => {
  ...
  const todayReadingMs = dailyReadingMsByDate[todayDateKey] || 0;
  ...
  const recentCompletedBooks = useMemo(() => {
    ...
    return safeBooks
      ...
      .slice(0, 5);
  }, [safeBooks, completedBookIds, completedAtByBookId]);
  ...
  const buildStickyNotePrompt = (historyMessages: StickyNoteHistoryMessage[]) => {
    const personaName = safeTrim(activePersona?.name) || '未设定用户';
    const personaDescription = safeTrim(activePersona?.description);
    const characterName = safeTrim(activeCharacter?.name) || '未设定角色';
    const characterDescription = safeTrim(activeCharacter?.description) || '（暂无角色人设描述）';

    const formatWorldBookSection = (entries: WorldBookEntry[]) => {
      if (entries.length === 0) return '';
      return entries
        .map((entry, index) => {
          const title = safeTrim(entry.title) || `条目 ${index + 1}`;
          const content = safeTrim(entry.content) || '（空）';
  ...
  return (
    <div className={`flex-1 flex flex-col p-6 pb-28 overflow-y-auto no-scrollbar ${containerClass}`}>
      <header className="mb-6 pt-2">
        <h1 className={`text-2xl font-black ${headingClass}`}>统计</h1>
        <p className={`mt-1 text-xs ${subTextClass}`}>阅读简介</p>
      </header>

      <section className="mb-8">
        <div className={`${cardClass} p-5 rounded-2xl`}>
          <div className="flex items-center justify-between">
            <span className={`text-sm font-semibold ${headingClass}`}>今日阅读</span>
            <span className="text-xs text-slate-400">目标 {goalMinutes} 分钟</span>
          </div>
          <div className={`mt-4 text-4xl font-black ${headingClass}`}>{formatReadingDuration(todayReadingMs)}</div>
          <div className={`mt-2 text-xs ${subTextClass}`}>
            连续 {consecutiveReadingDays} 天 · 累计 {cumulativeReadingHours.toFixed(1)}h · 目标书籍 {goalCompletedCount}/{goalTotalCount}
          </div>
          <div className={`mt-3 h-1.5 rounded-full overflow-hidden ${pressedClass}`}>
            <div className="h-full" style={{ width: `${todayGoalProgress * 100}%`, background: goalRingFillColor }} />
          </div>
        </div>
        ...
        <div className="mt-4 space-y-2">
          <button
            type="button"
            className={`${pressedCard === 'streak' ? pressedClass : cardClass} p-3 rounded-xl flex items-center justify-between text-left transition-all active:scale-[0.98]`}
            {...getCardEvents('streak')}
          >
            <div className="flex items-center gap-2">
              <Flame size={16} className="text-rose-400" />
              <span className={`text-sm font-semibold ${headingClass}`}>连续阅读</span>
            </div>
            <span className="text-sm text-slate-400">{consecutiveReadingDays} 天</span>
          </button>
          <button
            type="button"
            className={`${pressedCard === 'duration' ? pressedClass : cardClass} p-3 rounded-xl flex items-center justify-between text-left transition-all active:scale-[0.98]`}
            {...getCardEvents('duration')}
          >
            <div className="flex items-center gap-2">
              <Clock size={16} style={{ color: durationAccentColor }} />
              <span className={`text-sm font-semibold ${headingClass}`}>阅读最多</span>
            </div>
            <span className="text-sm text-slate-400">{cumulativeReadingHours.toFixed(1)}h</span>
          </button>
          <button
            type="button"
            className={`${pressedCard === 'completed' ? pressedClass : cardClass} p-3 rounded-xl flex items-center justify-between text-left transition-all active:scale-[0.98]`}
            {...getCardEvents('completed')}
          >
            <div className="flex items-center gap-2">
              <BookOpen size={16} style={{ color: completedAccentColor }} />
              <span className={`text-sm font-semibold ${headingClass}`}>最近读完</span>
            </div>
            <span className="text-sm text-slate-400">{totalCompletedBookCount} 本</span>
          </button>
        </div>
      </section>
      ...
      {openCardModal && (
        <ModalPortal>
          ...
          >
            <div
              className="ios-panel w-full max-w-sm h-[20rem] rounded-2xl p-6 relative flex flex-col"
      ...
      {stickyNoteError && (
        <ModalPortal>
          <div className="fixed inset-0 z-[130] flex items-center justify-center p-6 bg-slate-900/35 backdrop-blur-sm">
            <div
              className="ios-panel w-full max-w-xs rounded-2xl p-5"
            >
