...
import { getReaderThemeById } from '../utils/readerThemes';
...
const DEFAULT_READER_FONT_ID = SERIF_READER_FONT_ID;
...
const getDefaultReaderTypography = (darkMode: boolean): ReaderTypographyStyle => ({
  fontSizePx: 19,
  lineHeight: 1.9,
  textColor: darkMode ? '#D0D5DD' : '#4B4035',
  backgroundColor: darkMode ? '#0F1116' : '#F5EAD8',
  textAlign: 'left',
});
...
const Reader: React.FC<ReaderProps> = ({
  ...
  const lastReaderThemeIdRef = useRef<string | null>(null);
  const lastReaderDarkModeRef = useRef<boolean | null>(null);
  ...
  const [pageFlipState, setPageFlipState] = useState<{ active: boolean; direction: 'next' | 'prev' | null }>({
    active: false,
    direction: null,
  });
  ...
  const triggerPageFlip = useCallback((direction: 'next' | 'prev') => {
    if (pageFlipState.active) return;
    const scroller = readerScrollRef.current;
    if (!scroller) return;
    ...
    const lineHeightPx = Math.max(1, readerTypography.fontSizePx * (Number(readerTypography.lineHeight) || 1.6));
    const nextTopRaw = direction === 'next'
      ? Math.min(maxScroll, currentTop + pageHeight)
      : Math.max(0, currentTop - pageHeight);
    const snappedTop = Math.round(nextTopRaw / lineHeightPx) * lineHeightPx;
    const nextTop = Math.min(maxScroll, Math.max(0, snappedTop));

    if (nextTop === currentTop) {
      const switched = tryAutoSwitchChapter(direction);
      if (!switched) return;
    } else {
      scroller.scrollTop = nextTop;
      lastReaderScrollTopRef.current = nextTop;
    }

    setPageFlipState({ active: true, direction });
    if (pageFlipTimerRef.current) {
      window.clearTimeout(pageFlipTimerRef.current);
    }
    pageFlipTimerRef.current = window.setTimeout(() => {
      setPageFlipState({ active: false, direction: null });
    }, 420);
  }, [pageFlipState.active, tryAutoSwitchChapter, readerTypography.fontSizePx, readerTypography.lineHeight]);
  ...
  useEffect(() => {
    const nextThemeId = appSettings.readerMore.appearance.readerThemeId;
    const themeChanged = lastReaderThemeIdRef.current !== nextThemeId;
    const darkModeChanged = lastReaderDarkModeRef.current !== isDarkMode;

    if (!themeChanged && !darkModeChanged) return;

    lastReaderThemeIdRef.current = nextThemeId;
    lastReaderDarkModeRef.current = isDarkMode;

    const theme = getReaderThemeById(nextThemeId, isDarkMode);
    setReaderTypography(prev => ({
      ...prev,
      backgroundColor: theme.paperColor,
      textColor: theme.textColor,
    }));
  }, [appSettings.readerMore.appearance.readerThemeId, isDarkMode]);
  ...
  useEffect(() => {
    ...
    const hydrateReaderAppearance = async () => {
      try {
        ...
        let normalizedTypography: ReaderTypographyStyle = {
          fontSizePx: clamp(
        ...
        if (!isDarkMode) {
          const legacyLightBackgrounds = new Set(['#F9F4EC', '#F7F2E8']);
          const legacyLightTexts = new Set(['#3F3A2F', '#4A433A']);
          if (legacyLightBackgrounds.has(normalizedTypography.backgroundColor)) {
            normalizedTypography = { ...normalizedTypography, backgroundColor: defaults.backgroundColor };
          }
          if (legacyLightTexts.has(normalizedTypography.textColor)) {
            normalizedTypography = { ...normalizedTypography, textColor: defaults.textColor };
          }
        }
  ...
  const handleReaderWheel = (e: React.WheelEvent<HTMLDivElement>) => {
    ...
    if (isPageMode) {
      if (Math.abs(e.deltaY) > 6) {
        triggerPageFlip(e.deltaY > 0 ? 'next' : 'prev');
      }
      return;
    }
  ...
  const handleReaderTouchStart = (e: React.TouchEvent<HTMLDivElement>) => {
    ...
    const startY = startTouch?.clientY ?? null;
    const startX = startTouch?.clientX ?? null;
  ...
  const handleReaderTouchMove = (e: React.TouchEvent<HTMLDivElement>) => {
    ...
    const currentY = currentTouch?.clientY ?? touchStartYRef.current;
    const currentX = currentTouch?.clientX ?? touchStartXRef.current ?? 0;
  ...
  const handleReaderTouchEnd = () => {
    ...
    const endX = touchLastXRef.current ?? startX;
    const endY = touchLastYRef.current ?? startY;
    if (isPageMode && !touchSwitchHandledRef.current && !touchSwipeHandledRef.current && startX !== null && startY !== null && endX !== null && endY !== null) {
      ...
      if (absX > 64 && absX > absY * 1.25) {
        triggerPageFlip(deltaX < 0 ? 'next' : 'prev');
        touchSwipeHandledRef.current = true;
      } else if (absX < 8 && absY < 8) {
        const rect = readerScrollRef.current?.getBoundingClientRect();
        if (rect) {
          const ratio = (startX - rect.left) / rect.width;
          if (ratio < 0.28) {
  ...
  const handleReaderClick = (e: React.MouseEvent<HTMLDivElement>) => {
    if (!isPageMode || isHighlightMode) return;
  ...
  const typographyInputClass = `h-8 rounded-md px-2 text-[11px] outline-none reader-ios-input ${isDarkMode ? 'text-slate-200 placeholder-slate-500' : 'text-slate-700 placeholder-slate-400'}`;
  const typographySelectTriggerClass = `w-full h-8 rounded-md px-2 flex items-center justify-between cursor-pointer transition-all active:scale-[0.99] reader-ios-input ${isDarkMode ? 'text-slate-200' : 'text-slate-700'}`;
  const typographyIconButtonClass = `w-8 h-8 rounded-full flex items-center justify-center transition-all ${isDarkMode ? 'reader-ios-btn text-slate-300 hover:text-white' : 'reader-ios-btn text-slate-500 hover:text-slate-700'}`;
  const getTypographyAlignButtonClass = (value: ReaderTextAlign) => {
    ...
    if (isDarkMode) {
      return `h-8 flex-1 rounded-lg flex items-center justify-center gap-1.5 text-[11px] font-semibold transition-all active:scale-[0.98] ${
        isActive
          ? 'reader-ios-btn text-rose-200 ring-1 ring-rose-300/50'
          : 'reader-ios-btn text-slate-300 hover:text-white'
      }`;
    }
    return `h-8 flex-1 rounded-lg flex items-center justify-center gap-1.5 text-[11px] font-semibold transition-all active:scale-[0.98] ${
      isActive ? 'reader-ios-btn text-rose-500 ring-1 ring-rose-300/40' : 'reader-ios-btn text-slate-600 hover:text-slate-800'
    }`;
  };
  ...
  const readerTheme = getReaderThemeById(appSettings.readerMore.appearance.readerThemeId, isDarkMode);
  const readerMode = appSettings.readerMore.appearance.readingMode || 'scroll';
  const isPageMode = readerMode === 'page';
  const readerFabOffsetPx = Math.min(200, Math.max(0, Math.round(appSettings.readerMore.appearance.readerFabOffsetPx || 0)));
  const readerContentPaddingTop = isPageMode ? 56 : 36;
  const readerContentPaddingBottom = (isPageMode ? 220 : 160) + Math.max(0, safeAreaBottom || 0) + readerFabOffsetPx;
  const readerScrollStyle = {
    touchAction: isHighlightMode ? 'none' : isPageMode ? 'pan-x' : 'pan-y',
    backgroundColor: readerTypography.backgroundColor,
    overflowY: isPageMode ? 'hidden' : 'auto',
    paddingTop: `${readerContentPaddingTop}px`,
    paddingBottom: `${readerContentPaddingBottom}px`,
  ...
  const renderTypographyColorEditor = (kind: TypographyColorKind, label: string) => {
    ...
    return (
      <div className="rounded-xl p-2 reader-ios-subpanel">
  ...
  return (
    <div
      ref={readerRootRef}
      className={`reader-ios flex flex-col h-full min-h-0 relative overflow-hidden transition-colors duration-300 ${
        isDarkMode ? 'dark-mode' : ''
      }`}
      style={{
        paddingTop: `${Math.max(0, safeAreaTop)}px`,
        paddingBottom: `${Math.max(0, safeAreaBottom)}px`,
        backgroundColor: isDarkMode ? '#0F1116' : readerTheme.chromeColor,
        color: isDarkMode ? '#E2E8F0' : readerTheme.textColor,
      }}
    >
      {ttsErrorToast.show && (
        <div
        ...
        >
          <div
            className={`ios-panel w-[min(94vw,760px)] px-8 py-4 rounded-[28px] flex items-center gap-4 ${isDarkMode ? 'text-slate-200' : 'text-[#3F3A2F]'}`}
          >
      ...
      <div className="reader-topbar">
        <button onClick={handleBackClick} className="reader-toolbar-btn reader-ios-btn text-slate-500 hover:text-slate-700 shrink-0">
          <ArrowLeft size={20} />
        </button>
        <div className="reader-title-chip">
          <div className="text-xs font-serif font-medium truncate">{activeBook?.title || '\u9605\u8bfb\u4e2d'}</div>
        </div>
        <div className="reader-topbar-actions">
          <button
            onClick={toggleTocPanel}
            className="reader-toolbar-btn reader-ios-btn text-slate-500 hover:text-rose-400"
            title="\u76ee\u5f55"
          >
            <ListIcon size={18} />
          </button>
          <button
            ...
            className={`reader-toolbar-btn reader-ios-btn reader-tool-toggle ${isBookmarkModalOpen ? 'reader-tool-active' : ''}`}
          ...
          <button
            ...
            className={`reader-toolbar-btn reader-ios-btn reader-tool-toggle ${isHighlighterVisualActive ? 'reader-tool-active' : ''}`}
          ...
          <button
            ...
            className={`reader-toolbar-btn reader-ios-btn reader-tool-toggle ${isTypographyPanelOpen ? 'reader-tool-active' : ''}`}
            style={typographyToggleStyle}
            title={'\u6587\u5b57\u6837\u5f0f'}
          >
            <Type size={18} />
          ...
          <button
            ...
            className={`reader-toolbar-btn reader-ios-btn reader-tool-toggle ${isMoreSettingsOpen ? 'reader-tool-active' : ''}`}
            style={typographyToggleStyle}
            title="更多设置"
          >
        ...
        <>
          ...
          {isTocOpen && (
            <div
              ...
              className={`absolute z-50 right-4 w-[min(22rem,calc(100vw-2rem))] max-h-[32vh] overflow-hidden rounded-2xl p-2 reader-ios-panel ${closingFloatingPanel === 'toc' ? 'reader-flyout-exit' : 'reader-flyout-enter'} flex flex-col`}
              style={floatingPanelAnchorStyle}
            >
              <div className="px-1 pb-2">
                <div className="rounded-xl p-1 reader-ios-subpanel">
          ...
          {isHighlighterPanelOpen && (
            <div
              className={`absolute z-50 right-4 w-[min(22rem,calc(100vw-2rem))] max-h-[32vh] overflow-hidden rounded-2xl p-2 reader-ios-panel ${closingFloatingPanel === 'highlighter' ? 'reader-flyout-exit' : 'reader-flyout-enter'} flex flex-col`}
              style={floatingPanelAnchorStyle}
            >
              ...
              <div className="flex-1 overflow-y-auto no-scrollbar px-1 pb-1">
                <div className="mb-2 flex items-center gap-2">
                  <div className="h-10 flex-1 rounded-xl p-1.5 reader-ios-subpanel">
                  ...
                  <input
                    ...
                    maxLength={7}
                    spellCheck={false}
                    className={`h-10 w-28 rounded-lg font-mono text-xs uppercase text-center outline-none reader-ios-input ${isDarkMode ? 'text-slate-200' : 'text-slate-700'}`}
                  />
                </div>
              ...
              <div className="mt-1 flex gap-2 px-1 pb-1">
                <button
                  ...
                  className={`flex-1 h-7 rounded-full text-[11px] font-bold ${isDarkMode ? 'reader-ios-btn text-slate-300 hover:text-slate-100' : 'reader-ios-btn text-slate-600 hover:text-slate-800'}`}
                >
                  {'\u53d6\u6d88'}
                </button>
                <button
          ...
          {isTypographyPanelOpen && (
            <div
              className={`absolute z-50 right-4 w-[min(22rem,calc(100vw-2rem))] max-h-[32vh] overflow-hidden rounded-2xl p-2 reader-ios-panel ${closingFloatingPanel === 'typography' ? 'reader-flyout-exit' : 'reader-flyout-enter'} flex flex-col`}
              style={floatingPanelAnchorStyle}
            >
              <div className="text-xs font-bold uppercase tracking-wider text-slate-400 px-2 py-1">
                {'\u6587\u5b57\u6837\u5f0f'}
      ...
      {isBookmarkModalOpen && (
        <>
          ...
          />
          <div
            className={`absolute z-50 right-4 w-[min(22rem,calc(100vw-2rem))] rounded-2xl p-2 reader-ios-panel ${isBookmarkModalClosing ? 'reader-flyout-exit' : 'reader-flyout-enter'}`}
            style={floatingPanelAnchorStyle}
          >
            ...
            <div className="px-1 pb-1">
              <input
                ...
                placeholder={'\u8bf7\u8f93\u5165\u4e66\u7b7e\u540d\u79f0'}
                className={`w-full h-10 rounded-xl px-3 text-sm outline-none reader-ios-input ${isDarkMode ? 'text-slate-200 placeholder-slate-500' : 'text-slate-700 placeholder-slate-400'}`}
              />
              <div className="mt-3 flex items-center gap-2">
                <button
                  type="button"
                  onClick={closeBookmarkModal}
                  className={`flex-1 h-7 rounded-full text-[11px] font-bold ${isDarkMode ? 'reader-ios-btn text-slate-300 hover:text-slate-100' : 'reader-ios-btn text-slate-500 hover:text-slate-700'}`}
                >
                  {'\u53d6\u6d88'}
                </button>
                <button
      ...
      <div ref={readerViewportContainerRef} className="relative flex-1 min-h-0 m-4 mt-0">
        {pageFlipState.active && pageFlipState.direction && (
          <div className={`reader-page-flip ${pageFlipState.direction}`} aria-hidden />
        )}
        <div
          ref={readerScrollRef}
          aria-busy={isLoadingMaskVisible}
          className="reader-scroll-panel reader-content-scroll reader-ios-paper relative h-full min-h-0 overflow-y-auto rounded-[22px] transition-colors px-6"
