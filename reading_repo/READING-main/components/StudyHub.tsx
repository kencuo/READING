...
import {
  ...
  Loader2, BookMarked, CheckCircle2, NotebookPen, CircleCheckBig, Highlighter,
...
import {
  ...
  StudyNoteCommentMessage, QuizSession, QuizConfig, QuizQuestion, ReaderCssPreset, ReaderHighlightRange,
...
type HighlightGroup = {
  bookId: string;
  title: string;
  items: HighlightSnippet[];
};

type AiUnderlineSnippet = {
  id: string;
  text: string;
  color: string;
  chapterTitle: string;
...
type AiUnderlineGroup = {
  bookId: string;
  title: string;
  items: AiUnderlineSnippet[];
};

const HIGHLIGHT_SNIPPET_MAX_LENGTH = 180;
...
const compactHighlightText = (value: string) => value.replace(/\s+/g, ' ').trim();

const clampNumber = (value: number, min: number, max: number) => Math.min(max, Math.max(min, value));

const buildHighlightSnippet = (text: string, start: number, end: number) => {
  const length = text.length;
  const safeStart = clampNumber(Math.min(start, end), 0, length);
  const safeEnd = clampNumber(Math.max(start, end), 0, length);
  if (safeEnd <= safeStart) return null;
  const snippetRaw = compactHighlightText(text.slice(safeStart, safeEnd));
  if (!snippetRaw) return null;
  const snippet = snippetRaw.length > HIGHLIGHT_SNIPPET_MAX_LENGTH
    ? `${snippetRaw.slice(0, HIGHLIGHT_SNIPPET_MAX_LENGTH).trimEnd()}...`
    : snippetRaw;
  return { snippet, start: safeStart, end: safeEnd };
};

const extractHighlightSnippets = (params: {
  bookId: string;
  chapterTitle: string;
  chapterIndex: number | null;
  text: string;
  ranges: ReaderHighlightRange[];
}) => {
  const { bookId, chapterTitle, chapterIndex, text, ranges } = params;
  const items: HighlightSnippet[] = [];
  const seen = new Set<string>();

  ranges.forEach((range, index) => {
    const snippetInfo = buildHighlightSnippet(text, range.start, range.end);
    if (!snippetInfo) return;
    const signature = `${chapterIndex ?? 'full'}:${snippetInfo.start}-${snippetInfo.end}:${snippetInfo.snippet}`;
    if (seen.has(signature)) return;
    seen.add(signature);
    items.push({
      id: `${bookId}-${chapterIndex ?? 'full'}-${snippetInfo.start}-${snippetInfo.end}-${index}`,
      text: snippetInfo.snippet,
      color: range.color || DEFAULT_HIGHLIGHT_COLOR,
      chapterTitle,
      chapterIndex,
      start: snippetInfo.start,
      end: snippetInfo.end,
    });
  });
...
const StudyHub: React.FC<StudyHubProps> = ({
  ...
  const activeBtnClass = 'active:shadow-[var(--ios-inset)] active:translate-y-px';
  const disabledIconButtonClass = `${btnClass} ${isDarkMode ? 'text-slate-500' : 'text-slate-400'} opacity-55 cursor-not-allowed`;
  const enabledDangerIconButtonClass = `${isDarkMode ? 'text-[#f3a4b2]' : 'text-[#bf616b]'} ${btnClass} ${activeBtnClass}`;
  ...
  const [aiUnderlineGroups, setAiUnderlineGroups] = useState<AiUnderlineGroup[]>([]);
  ...
  const buildHighlightGroupsForBooks = useCallback(async (targetBooks: Book[]) => {
    ...
    for (const book of targetBooks) {
      ...
      const fullNormalizedText = normalizeReaderLayoutText(
        stored.fullText || (normalizedChapters.length > 0 ? normalizedChapters.join('') : '')
      );
      const items: HighlightSnippet[] = [];
      ...
      items.sort((a, b) => {
        const aChapter = a.chapterIndex ?? Number.MAX_SAFE_INTEGER;
        const bChapter = b.chapterIndex ?? Number.MAX_SAFE_INTEGER;
        if (aChapter !== bChapter) return aChapter - bChapter;
        if (a.start !== b.start) return a.start - b.start;
        return a.end - b.end;
      });
      groups.push({ bookId: book.id, title: book.title, items });
    }

    return groups;
  }, []);

  const buildAiUnderlineGroupsForBooks = useCallback(async (targetBooks: Book[]) => {
    const groups: AiUnderlineGroup[] = [];
    ...
    for (const book of targetBooks) {
      ...
      const items: AiUnderlineSnippet[] = [];
      ...
      Object.entries(aiUnderlineMap).forEach(([key, ranges]) => {
        ...
        if (key === 'full') {
          if (!fullNormalizedText) return;
          items.push(...extractAiUnderlineSnippets({
            bookId: book.id,
            chapterTitle: '全书',
            chapterIndex: null,
            text: fullNormalizedText,
            ranges: ranges as ReaderAiUnderlineRange[],
          }));
          return;
        }
  ...
  const extractAiUnderlineSnippets = (params: {
    bookId: string;
    chapterTitle: string;
    chapterIndex: number | null;
    text: string;
    ranges: ReaderAiUnderlineRange[];
  }) => {
    const { bookId, chapterTitle, chapterIndex, text, ranges } = params;
    const items: AiUnderlineSnippet[] = [];
    const seen = new Set<string>();
    const DEFAULT_AI_UNDERLINE_COLOR = '#A0C4FF';
    ...
    ranges.forEach((range, index) => {
      ...
      seen.add(signature);
      items.push({
        id: `${bookId}-ai-${chapterIndex ?? 'full'}-${snippetInfo.start}-${snippetInfo.end}-${index}`,
        text: snippetInfo.snippet,
        color: range.color || DEFAULT_AI_UNDERLINE_COLOR,
        chapterTitle,
        chapterIndex,
        start: snippetInfo.start,
        end: snippetInfo.end,
  ...
  const handleOpenHighlights = async () => {
    ...
    if (boundBooks.length === 0) {
      showNotification('当前笔记本未绑定书籍', 'error');
      return;
    }
    setHighlightLoading(true);
    setShowHighlights(true);
    try {
      const [groups, aiGroups] = await Promise.all([
        buildHighlightGroupsForBooks(boundBooks),
        buildAiUnderlineGroupsForBooks(boundBooks),
      ]);
      setHighlightGroups(groups);
      setAiUnderlineGroups(aiGroups);
    } catch {
      setHighlightGroups([]);
      setAiUnderlineGroups([]);
    } finally {
    ...
    for (const charId of selectedCharIds) {
      ...
      try {
        ...
        const prompt = buildNoteCommentPrompt({
          ...
          bookContexts,
          ragContextByBookId,
          activeBookIds: activeNotebook.boundBookIds,
        });
    ...
    if (persona && character) {
      ...
      try {
        ...
        const prompt = buildQuizOverallCommentPrompt({
          ...
          bookTitles,
          activeBookIds: session.config.bookIds,
        });
        console.log('[StudyHub Prompt]', prompt);
  ...
  const handleRefreshQuizOverallComment = async () => {
    ...
    try {
      ...
      const prompt = buildQuizOverallCommentPrompt({
        ...
        bookTitles,
        activeBookIds: activeQuizSession.config.bookIds,
      });
      console.log('[StudyHub Prompt]', prompt);
      const comment = await callAiModel(prompt, apiConfig);
      ...
      <ModalPortal>
        ...
        >
          <div onClick={(e) => e.stopPropagation()}
            className={`ios-panel w-full max-w-sm rounded-2xl px-2 py-5 relative flex flex-col ${closingCreateModal ? 'app-fade-exit' : 'app-fade-enter'}`}
  ...
  const renderPaperModal = () => {
    ...
    return (
      <ModalPortal>
        ...
        >
          <div onClick={(e) => e.stopPropagation()}
            className={`ios-panel w-full max-w-sm max-h-[85vh] rounded-2xl px-2 py-5 relative flex flex-col overflow-y-auto no-scrollbar ${closingPaperModal ? 'app-fade-exit' : 'app-fade-enter'}`}
          >
  ...
  const renderEditModal = () => {
    ...
    return (
      <ModalPortal>
        ...
        >
          <div onClick={(e) => e.stopPropagation()}
            className={`ios-panel w-full max-w-sm rounded-2xl px-2 py-5 relative flex flex-col ${closingEditModal ? 'app-fade-exit' : 'app-fade-enter'}`}
            style={{ maxHeight: 'calc(var(--app-screen-height) - 5rem)' }}
          >
  ...
  const renderQuizConfigModal = () => {
    ...
    return (
      <ModalPortal>
        ...
        >
          <div onClick={(e) => e.stopPropagation()}
            className={`ios-panel w-full max-w-sm rounded-2xl px-2 py-5 relative flex flex-col ${closingQuizConfigModal ? 'app-fade-exit' : 'app-fade-enter'}`}
            style={{ maxHeight: 'calc(var(--app-screen-height) - 5rem)' }}
          >
  ...
  const renderHighlightModal = () => {
    if (!showHighlights || !activeNotebook) return null;
    const totalHighlightCount = highlightGroups.reduce((sum, group) => sum + group.items.length, 0);
    const totalAiUnderlineCount = aiUnderlineGroups.reduce((sum, group) => sum + group.items.length, 0);

    return (
      <ModalPortal>
        <div
          className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-500/20 backdrop-blur-sm app-fade-enter"
          onClick={() => setShowHighlights(false)}
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="ios-panel w-full max-w-lg max-h-[85vh] rounded-2xl px-2 py-5 relative flex flex-col overflow-y-auto no-scrollbar"
          >
            <button
              onClick={() => setShowHighlights(false)}
            ...
            <div className="px-4 flex items-center justify-between mb-4">
              <h3 className={`text-lg font-bold ${headingClass}`}>摘录笔记</h3>
              <span className={`text-xs ${subTextClass}`}>
                {highlightLoading ? '加载中...' : `手动 ${totalHighlightCount} / AI ${totalAiUnderlineCount}`}
              </span>
            </div>
            ...
            <div className="px-4 pb-2 space-y-6">
              ...
              {!highlightLoading && highlightGroups.length > 0 && (
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-sm font-semibold ${headingClass}`}>手动高亮</span>
                    <span className={`text-xs ${subTextClass}`}>{totalHighlightCount} 条</span>
                  </div>
                  {highlightGroups.map((group) => (
                    <div key={group.bookId} className="space-y-2">
                      <div className="flex items-center justify-between">
                        <span className={`text-xs font-medium ${subTextClass}`}>{group.title}</span>
                      </div>
              ...
              {/* AI Underlines */}
              {!highlightLoading && aiUnderlineGroups.length > 0 && (
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    <Feather size={14} className="text-blue-400" />
                    <span className={`text-sm font-semibold ${headingClass}`}>AI 划线</span>
                    <span className={`text-xs ${subTextClass}`}>{totalAiUnderlineCount} 条</span>
                  </div>
                  {aiUnderlineGroups.map((group) => (
                    <div key={group.bookId} className="space-y-2">
                      <div className="flex items-center justify-between">
                        <span className={`text-xs font-medium ${subTextClass}`}>{group.title}</span>
                      </div>
                      <div className="space-y-2">
                        {group.items.map((item) => (
                          <div
                            key={item.id}
                            className={`rounded-xl p-3 border ${cardClass} ${isDarkMode ? 'border-slate-700/40' : 'border-blue-200/50'}`}
                          >
                            <div className={`flex items-center gap-2 text-[10px] ${subTextClass}`}>
                              <span
                                className="w-2 h-2 rounded-full"
                                style={{ backgroundColor: item.color || '#A0C4FF' }}
                              />
                              <span>{item.chapterTitle}</span>
                            </div>
                            <p
              ...
              {!highlightLoading && highlightGroups.length === 0 && aiUnderlineGroups.length === 0 && (
                <div className="text-center text-sm text-slate-400 py-10">暂无摘录</div>
              )}
            </div>
          </div>
        </div>
