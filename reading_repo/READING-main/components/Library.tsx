...
import { Book as BookIcon, Plus, Clock, Calendar, Edit2, Check, UserCircle, LogOut, Link2, Search, Filter, MoreVertical, X, Image, Trash2, Link, FileText, FileUp, List, Sparkles, AlertTriangle, ArrowUpDown, ArrowUp, ArrowDown, LayoutGrid, AlignJustify, HelpCircle, ChevronDown } from 'lucide-react';
...
const DEFAULT_DAILY_READING_GOAL_MINUTES = 5;
...
const Library: React.FC<LibraryProps> = ({ 
  ...
  const containerClass = 'ios-bg ios-text';
  const headingClass = isDarkMode ? 'text-slate-100' : 'text-[#2F2A23]';
  const subTextClass = isDarkMode ? 'text-slate-400' : 'text-[#6B635A]';
  const cardClass = 'ios-card';
  const pressedClass = 'ios-pressed';
  const inputClass = 'ios-input';
  const btnClass = 'ios-btn';
  const compactEditButtonClass = `w-8 h-8 rounded-full flex items-center justify-center text-slate-500 hover:text-rose-400 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all duration-150 active:scale-95 ${btnClass}`;
  ...
  const [isGoalModalClosing, setIsGoalModalClosing] = useState(false);
  ...
  const [importingBook, setImportingBook] = useState<Partial<Book>>({
      title: '', author: '', coverUrl: '', tags: [], fullText: '', chapterRegex: '', boundPersonaId: undefined, boundCharacterId: undefined
  });
  ...
  useEffect(() => {
    try {
      ...
      setGoalBookIds(parsed.filter((item) => typeof item === 'string' && item.trim()));
    } catch {
      setGoalBookIds([]);
    }
  }, [books.length]);
  ...
  const goalModalCloseTimerRef = useRef<number | null>(null);
  ...
  useEffect(() => {
    return () => {
      ...
      if (goalModalCloseTimerRef.current) window.clearTimeout(goalModalCloseTimerRef.current);
    };
  }, []);
  ...
  const handleSelectCharacter = (charId: string | null) => {
    ...
    if (charId) {
       ...
       if (selectedChar) {
          const boundPersona = personas.find(p => (p.boundCharacterIds || []).includes(selectedChar.id));
          if (boundPersona) {
  ...
  const openGoalModal = () => {
    if (goalModalCloseTimerRef.current) {
      window.clearTimeout(goalModalCloseTimerRef.current);
      goalModalCloseTimerRef.current = null;
    }
    setGoalBookSearchKeyword('');
    setIsGoalModalClosing(false);
    setIsGoalModalOpen(true);
  };

  const closeGoalModal = () => {
    if (!isGoalModalOpen) return;
    setIsGoalModalClosing(true);
    if (goalModalCloseTimerRef.current) window.clearTimeout(goalModalCloseTimerRef.current);
    goalModalCloseTimerRef.current = window.setTimeout(() => {
  ...
  const saveImportBook = async () => {
     if (importingBook.title) {
        ...
        const newBook: Book = {
            ...
            boundPersonaId: importingBook.boundPersonaId,
            boundCharacterId: importingBook.boundCharacterId,
        };
...
输出: ^Chapter\\s*\\d+`;
    ...
    return (
      <div className="overflow-y-auto no-scrollbar flex-1 -mx-2 px-2 space-y-5 pb-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">绑定角色</label>
            <select
              value={book.boundCharacterId || ''}
              onChange={(e) => {
                const value = e.target.value || undefined;
                if (isEdit) {
                  setEditingBook({ ...editingBook!, boundCharacterId: value });
                } else {
                  setImportingBook({ ...importingBook, boundCharacterId: value });
                }
              }}
              className={`w-full px-4 py-3 text-sm rounded-xl outline-none ${inputClass}`}
            >
          ...
          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">绑定用户</label>
            <select
              value={book.boundPersonaId || ''}
              onChange={(e) => {
                const value = e.target.value || undefined;
                if (isEdit) {
                  setEditingBook({ ...editingBook!, boundPersonaId: value });
                } else {
                  setImportingBook({ ...importingBook, boundPersonaId: value });
                }
              }}
              className={`w-full px-4 py-3 text-sm rounded-xl outline-none ${inputClass}`}
            >
  ...
  const dailyReadingMap = dailyReadingMsByDate || {};
  const todayDateKey = formatDateKey(new Date());
  const todayReadingMs = dailyReadingMap[todayDateKey] || 0;
  const goalMinutes = DEFAULT_DAILY_READING_GOAL_MINUTES;
  const todayGoalProgress = Math.min(1, todayReadingMs / (goalMinutes * 60 * 1000));
  const todayProgressPercent = Math.round(todayGoalProgress * 100);
  const todayReadingMinutes = Math.floor(todayReadingMs / 60000);
  const goalRingDegrees = Math.round(todayGoalProgress * 360);
  const goalRingBaseColor = isDarkMode ? 'rgba(148, 163, 184, 0.22)' : 'rgba(148, 163, 184, 0.18)';
  const goalRingFillColor = isDarkMode ? 'rgba(244, 114, 182, 0.85)' : 'rgba(226, 138, 157, 0.8)';
  const goalRingStyle = {
    background: `conic-gradient(${goalRingFillColor} ${goalRingDegrees}deg, ${goalRingBaseColor} 0deg)`,
  ...
  const weekCells = WEEKDAY_LABELS.map((label, index) => {
    ...
    return {
      ...
      readingMs: dailyReadingMap[dateKey] || 0,
    };
  });
  ...
  const goalBookLabelById = useMemo(() => {
    ...
    books.forEach((book) => {
      labelById.set(book.id, book.title || 'Untitled Book');
    });
    return labelById;
  }, [books]);
  ...
  const goalBookItems = useMemo(
    () => books.map((book) => {
      const tags = Array.isArray(book.tags)
        ? book.tags.filter((tag) => typeof tag === 'string' && tag.trim())
        : [];
      return {
        id: book.id,
  ...
  const filteredGoalBookItems = useMemo(() => {
    ...
    return goalBookItems.filter((book) => {
      const searchable = [book.label, book.author, book.tags.join(' ')].join(' ').toLowerCase();
      return searchable.includes(normalizedGoalBookSearch);
    });
  ...
  return (
    <>
      <div className={`flex-1 flex flex-col p-6 pb-28 overflow-y-auto no-scrollbar ${containerClass}`}>
        <header className="flex items-start justify-between gap-4 mb-6 pt-2">
          <div className="flex-1 pr-4 min-w-0">
            <h1 className={`text-3xl font-black ${headingClass}`}>主页</h1>
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
              <div className="flex flex-col items-center gap-1">
                <div className="w-12 h-12 rounded-full p-[3px]" style={goalRingStyle}>
                  <div className={`w-full h-full rounded-full flex items-center justify-center text-xs font-bold ${pressedClass}`}>
                    {todayReadingMinutes}
                  </div>
                </div>
                <span className="text-[10px] text-slate-400">{goalMinutes}分钟</span>
              </div>
              ...
              <div className="relative" ref={charMenuRef}>
                  ...
                  >
                     <div className={`relative w-12 h-12 rounded-full flex items-center justify-center overflow-hidden border-2 border-transparent transition-all group-hover:border-rose-300 ${btnClass}`}>
                       {renderAvatar(activeCharacter?.avatar, false, !activeCharacterId, 'CHAR')}
                     </div>
                     <div className="absolute -top-1 -right-1 w-5 h-5 rounded-full flex items-center justify-center text-[10px] shadow-sm z-10 border border-white/10 ios-panel text-blue-400">
                        <FeatherIcon size={12} />
                     </div>
        ...
        {recentBook && (
          <div className="mb-10">
            <div className="flex items-center justify-between mb-4">
              <h2 className={`text-base font-bold ${headingClass}`}>之前读过</h2>
              <span className="text-sm text-slate-300">›</span>
            </div>
            <div className="flex gap-4 overflow-x-auto pb-2 no-scrollbar">
              <div
                onClick={() => onOpenBook(recentBook)}
                className={`${cardClass} app-card-press min-w-[280px] p-4 flex gap-4 cursor-pointer rounded-2xl relative group`}
              >
                <div className="w-16 h-24 flex-shrink-0 rounded-lg overflow-hidden shadow-md app-card-press-media relative">
                  {recentBook.coverUrl ? (
                ...
                <div className="flex flex-col justify-between flex-1">
                  <div>
                    <h3 className={`text-base font-bold line-clamp-1 ${headingClass}`}>{recentBook.title}</h3>
                    <p className={`text-xs ${subTextClass}`}>{recentBook.author}</p>
                  </div>
                  <div>
                     <div className="flex justify-between text-[11px] text-slate-400 mb-2">
                       <span>已读 {recentBook.progress}%</span>
                       <span><Clock size={11} className="inline mr-1"/>{formatLastReadTime(recentBook.lastReadAt, recentBook.lastRead)}</span>
                     </div>
                ...
                <button
                  ...
                  className={`absolute top-3 right-3 ${compactEditButtonClass}`}
                >
                  <Edit2 size={14} />
                </button>
              </div>
        ...
        <section className="mb-10">
          <div className="text-center">
            <h2 className={`text-lg font-bold ${headingClass}`}>阅读目标</h2>
            <div className="flex items-center justify-center gap-2 text-xs text-rose-400">
              <button type="button" onClick={openGoalModal} className="hover:text-rose-500">设定目标</button>
              <span className="text-slate-300">·</span>
              <span className="text-slate-400">{goalCompletedCount}/{goalTotalCount}</span>
            </div>
            <p className={`mt-2 text-xs ${subTextClass}`}>找一本好书，设定一个目标，养成每天阅读的习惯。</p>
          </div>
          <div className="mt-6 flex flex-col items-center">
            <div className="relative w-56 h-56 rounded-full p-[10px]" style={goalRingStyle}>
              <div className={`absolute inset-[10px] rounded-full ${pressedClass}`} />
              <div className="relative z-10 h-full flex flex-col items-center justify-center">
                <div className={`text-5xl font-black ${headingClass}`}>{formatReadingDuration(todayReadingMs)}</div>
                <div className={`mt-2 text-xs ${subTextClass}`}>(目标 {goalMinutes} 分钟)</div>
                <div className={`mt-1 text-[10px] ${subTextClass}`}>{todayProgressPercent}%</div>
              </div>
            </div>
            <button
              type="button"
              onClick={() => librarySectionRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' })}
              className={`mt-6 w-full max-w-xs h-11 rounded-full text-sm font-semibold ${isDarkMode ? 'bg-white text-slate-900' : 'bg-slate-900 text-white'}`}
            >
              探索书库
            </button>
            <div className="mt-6 w-full">
              ...
              <div className="grid grid-cols-7 gap-2">
                {weekCells.map((cell) => {
                  const hasReading = cell.readingMs > 0;
                  const baseColor = hasReading
                    ? 'rgba(226, 138, 157, 0.18)'
                    : (isDarkMode ? 'rgba(148, 163, 184, 0.12)' : 'rgba(148, 163, 184, 0.08)');
                  const ringStyle = cell.isToday ? 'ring-2 ring-rose-300/70' : '';
                  return (
                    <div key={cell.dateKey} className="flex flex-col items-center gap-1">
                      <div
                        className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-semibold ${pressedClass} ${ringStyle}`}
                        style={{ backgroundColor: baseColor }}
                      >
                        {cell.day}
                      </div>
                      <div className="text-[10px] text-slate-400">{cell.label}</div>
                    </div>
                  );
                })}
              </div>
              <div className="mt-3 text-center text-xs text-slate-400">开启连续阅读新记录</div>
            </div>
          </div>
        </section>

        {/* Grid Header with Search & Filter & Sort */}
        <div ref={librarySectionRef} className="mb-4">
          <h2 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-3 pl-1">
             {isSearching ? '搜索结果' : '书库'}
          </h2>
          <div className="flex gap-2 mb-4">
             {/* Search Bar */}
             <div className={`flex-1 flex items-center px-3 py-2 rounded-full gap-2 ${inputClass}`}>
      ...
      {isEditModalOpen && editingBook && (
        <ModalPortal>
          ...
          <div className={`ios-panel w-full max-w-sm rounded-2xl p-6 relative flex flex-col ${closingModal === 'edit' ? 'app-fade-exit' : 'app-fade-enter'}`} style={{ maxHeight: 'calc(var(--app-screen-height) - 9rem)' }}>
      ...
      {isImportModalOpen && (
         <ModalPortal>
           <div className={`fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-500/20 backdrop-blur-sm ${closingModal === 'import' ? 'app-fade-exit' : 'app-fade-enter'}`}>
            <div className={`ios-panel w-full max-w-sm rounded-2xl p-6 relative flex flex-col ${closingModal === 'import' ? 'app-fade-exit' : 'app-fade-enter'}`} style={{ maxHeight: 'calc(var(--app-screen-height) - 9rem)' }}>
      ...
      {isGoalModalOpen && (
        <ModalPortal>
          <div
            className={`fixed inset-0 z-[105] flex items-center justify-center p-6 bg-slate-500/20 backdrop-blur-sm ${isGoalModalClosing ? 'app-fade-exit' : 'app-fade-enter'}`}
            onClick={closeGoalModal}
          >
            <div
              className={`ios-panel w-full max-w-sm rounded-2xl p-6 relative flex flex-col ${isGoalModalClosing ? 'app-fade-exit' : 'app-fade-enter'}`}
              style={{ maxHeight: 'calc(var(--app-screen-height) - 9rem)' }}
              onClick={(event) => event.stopPropagation()}
            >
              <button onClick={closeGoalModal} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600">
                <X size={20} />
              </button>

              <h3 className={`text-lg font-bold mb-4 text-center ${headingClass}`}>设定目标</h3>
              ...
              <div className={`mt-3 rounded-xl px-3 py-2 flex items-center gap-2 ${inputClass}`}>
                ...
                {goalBookSearchKeyword && (
                  <button type="button" onClick={() => setGoalBookSearchKeyword('')} className="text-slate-400 hover:text-slate-500">
                    <X size={14} />
                  </button>
                )}
              </div>
