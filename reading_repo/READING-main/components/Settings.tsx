...
import type { Book, RagPreset } from '../types';
...
const Settings: React.FC<SettingsProps> = ({ 
  ...
  const theme: ThemeClasses = {
    containerClass: 'ios-bg ios-text',
    headingClass: isDarkMode ? 'text-slate-100' : 'text-[#2F2A23]',
    cardClass: 'ios-card',
    pressedClass: 'ios-pressed',
    sectionIconClass: 'w-12 h-12 rounded-full flex items-center justify-center ios-pressed',
    inputClass: 'ios-input',
    btnClass: 'ios-btn',
    activeBorderClass: 'border-2 border-rose-300/60 relative z-20',
  ...
  const renderAvatarModal = () => {
    ...
    return (
      <ModalPortal>
        ...
        <div className="ios-panel w-full max-w-sm rounded-2xl p-6 relative">
  ...
  const storageLegendCardClass = 'ios-card';
