...
export const buildCharacterWorldBookSections = (
  ...
  activeBookIds?: string[] | string | null
  ...
  const normalizedBookIds = Array.isArray(activeBookIds)
    ? activeBookIds.filter(Boolean)
    : activeBookIds
      ? [activeBookIds]
      : [];

  const matchesCharacter = (entry: WorldBookEntry) => {
    const boundIds = (entry.boundCharacterIds || []).filter(Boolean);
    if (boundIds.length === 0) return true;
    if (!activeCharacter?.id) return false;
    return boundIds.includes(activeCharacter.id);
  };

  const matchesBook = (entry: WorldBookEntry) => {
    const boundIds = (entry.boundBookIds || []).filter(Boolean);
    if (boundIds.length === 0) return true;
    if (normalizedBookIds.length === 0) return false;
    return boundIds.some((id) => normalizedBookIds.includes(id));
  };

  const scopedEntries = worldBookEntries.filter(
    (entry) => boundCategories.has(entry.category) && matchesCharacter(entry) && matchesBook(entry)
  );
  return {
