...
const buildWorldBookPromptSection = (
  character: Character,
  ...
  const wbSections = buildCharacterWorldBookSections(character, worldBookEntries, activeBookIds);
  return {
...
export const buildNoteCommentPrompt = (params: BuildNoteCommentPromptParams): string => {
  ...
  const wb = buildWorldBookPromptSection(character, worldBookEntries, character.name, userPersona.name, activeBookIds);
...
export const buildQuizOverallCommentPrompt = (params: BuildQuizOverallCommentParams): string => {
  const { userPersona, character, worldBookEntries, questions, userAnswers, bookTitles, activeBookIds } = params;
